<?php
  ...
  class Solution {
    ...
    function areaOfMaxDiagonal($dimensions) {
      ...
      $highestDiag = 0;
      $result = null;
      foreach ($dimensions as $rectangle) {
        $length = $rectangle[0];
        $breadth = $rectangle[1];

        $area = $length * $breadth;

        $diagonal = pow ((($length ** 2) + ($breadth ** 2)), 0.5);

        if ($diagonal > $highestDiag) {
          $highestDiag = $diagonal;
          $result = $area;


        }elseif ($highestDiag == $diagonal) {
  ...
  echo $solution -> areaOfMaxDiagonal($dimensions);
